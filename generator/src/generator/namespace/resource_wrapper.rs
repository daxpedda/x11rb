...
fn generate_creator(
    ...
    let request_ns = request_ext.map(|ext_name| generator.module.namespace(ext_name).unwrap_or_else(|| {
            panic!(
    ...
    let has_feature = request_ns.as_ref().map(|ns| ext_has_feature(&ns.header)).unwrap_or(false);
    ...
    let (function_raw_name, function_name) = match request_ext {
        Some(_) => (format!("{}_{}", request_ns.header, function_raw_name), format!("super::{}::{}", request_ns.header, function_raw_name)),
